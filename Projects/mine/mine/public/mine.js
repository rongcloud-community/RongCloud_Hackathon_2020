...
var choose = new Array(2).fill(0);
var isfinish = 0;
var type=0;
var counts=0;
var resqqq = new Array();   //先声明一维
function init_resqqq(){
	for(var i=0;i<16;i++){        //一维长度为10
		resqqq[i]=new Array();    //在声明二维
    for(var j=0;j<30;j++) {      //二维长度为20
    	    resqqq[i][j]=0;
    	}
	}
}

function test(paneheight,panewidth,cellArr,tmppos,reClick){ //reClick用户告诉ai点击无效,值为-1是表示失败
    for(var i=1;i<=paneheight;i++){
		for (var j = 1; j <= panewidth; j++) {
			if(cellArr[j][i].isOpened == true){
				tmppos[0]=j;
				tmppos[1]=i;
				if(that.calAround(tmppos)==0)//已经打开的周围为0个雷时标记为-1
					resqqq[i-1][j-1]=-1;
				else
					resqqq[i-1][j-1]=that.calAround(tmppos);
			}else{
				if(cellArr[j][i].tag==1)
					resqqq[i-1][j-1]=-4;
			}
		};
	}
	//console.log(resqqq);
	if(reClick==0)
		websocket.send(JSON.stringify({res: resqqq,gameover: isfinish,reClick: 0}));  
	else
		websocket.send(JSON.stringify({res: resqqq,gameover: isfinish,reClick: 1}));
	// if(isfinish==2){
	// 	websocket.send(JSON.stringify({res: resqqq,gameover: isfinish}));
	// 	isfinish=0;
	// }
	}

websocket.onopen = function(){  //发送请求
	console.log("open");
};
...
var Mine = function  (ele,faceele,panewidth,paneheight,minenum,tagele,timeele) {
 ...
 Mine.prototype = {
	...
	triggerClick:function(pos,e){
		...
		//调试1，产生位置和数字
		console.log("点击位置："+pos[1]+","+pos[0]);
		console.log("该位置数字："+this.calAround(pos));
		...
		if(e && e.button == 2){//右键 标记雷	
			...
			if(tag == 0){
				...
				//console.log('tat is 1');
				this.drawCell(pos,4);
				theCell.tag = 2;
				this.notTaged ++;
				this.numToImage(this.notTaged,this.tagele);		
			}else if(tag == 2){
				//console.log('tat is 2');
				this.drawCell(pos,1);
				theCell.tag = 0;
			}
			return;
		...
		if(openNum == okNum){
			...
			this.ele.onmousedown = '';
			this.ele.onmousemove = '';	
			var word = prompt("请留下名字吧","");
			word += "获得胜利:耗时"+this.time+"s";
			send(word);
		}

		//调试2产生可见的结果
		//console.log(this.cellArr);
		// let arr=new Array(this.panewidth).fill(0);这种初始化会集体赋值
		// let resqq=new Array(this.paneheight).fill(arr);
		// var resqqq = new Array();   //先声明一维
        // for(var i=0;i<this.paneheight;i++){        //一维长度为10
        // resqqq[i]=new Array();    //在声明二维
        // for(var j=0;j<this.panewidth;j++){      //二维长度为20
        //  resqqq[i][j]=-1;
        // }
        // }
		var tmppos=new Array(2).fill(0);
		that=this;
		test(this.paneheight,this.panewidth,this.cellArr,tmppos,0);
		// for(var i=1;i<=this.paneheight;i++){
		// 	for (var j = 1; j <= this.panewidth; j++) {
		// 		if(this.cellArr[j][i].isOpened == true){
		// 			tmppos[0]=j;
		// 			tmppos[1]=i;
		// 			//console.log(that.calAround(tmppos));
		// 			resqqq[i-1][j-1]=that.calAround(tmppos);
		// 		}
		// 	};
		// }
	...
	aitriggerClick:function(pos,type){
		var theCell = this.cellArr[pos[0]][pos[1]];
		//调试1，产生位置和数字
		//console.log("点击位置："+pos[1]+","+pos[0]);
		//console.log("该位置数字："+this.calAround(pos));
		...
		if(theCell.isOpened == true||type==2){ //已经打开过的不做处理,发送一个包告诉ai type==2代表放弃插旗
			var tmppos=new Array(2).fill(0);
			that=this;
			test(this.paneheight,this.panewidth,this.cellArr,tmppos,1);
			return;
		}
		var tag = theCell.tag;
		if(type==1){	
	...
	createMines:function(pos){	//生成雷的位置。保存到一个数组[[2,3],[4,6]];	
		...
		for (var i = 0; i < minenum; i++) {
			...
			}while(in_array(mineItem,mineArr)||(mineItem[1]==1&&mineItem[0]%2==0)||(mineItem[1]==1&&(getRandom(10)%2!=0||getRandom(10)%3!=0))||pos.toString()== mineItem.toString());
...
websocket.onmessage = function (event) {
	...
	mine1.aiclick(choose[0]+1,choose[1]+1,data['type']);
	counts=counts+1;
