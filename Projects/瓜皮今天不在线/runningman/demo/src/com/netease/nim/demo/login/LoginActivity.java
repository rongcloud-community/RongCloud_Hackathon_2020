...
//                    login();
    ...
    private void login() {
        ...
        // 融云只提供消息通道，并不包含用户资料逻辑。开发者需要在管理后台或通过服务器接口将用户帐号和token同步到融云服务器。
        // 在这里直接使用同步到融云服务器的帐号和token登录。
        // 这里为了简便起见，demo就直接使用了密码的md5作为token。
        // 如果开发者直接使用这个demo，只更改appkey，然后就登入自己的账户体系的话，需要传入同步到融云服务器的token，而不是用户密码。
...
//                MainActivity.start(LoginActivity.this, null);
    ...
    /**
     * *********** 假登录示例：假登录后，可以查看该用户数据，但向融云发送数据会失败；随后手动登录后可以发数据 **************
     */
    public static volatile String token = "JwZuVE3qJ9fLYDI/ICAKW1XWzqNeTtWq@2x9v.cn.rongnav.com;2x9v.cn.rongcfg.com";
    private void fakeLoginTest() {
        ...
        RongIM.setUserInfoProvider(new RongIM.UserInfoProvider() {
            ...
            public UserInfo getUserInfo(String userId) {
                UserInfo userInfo = new UserInfo(userId, userId, Uri.parse("https://developer.rongcloud.cn/static/img/yunyinguser.jpg"));
                return userInfo;
            }
        ...
        // 进入主界面，此时可以查询数据（最近联系人列表、本地消息历史、群资料等都可以查询，但当融云服务器发起请求会返回408超时）
//        MainActivity.start(LoginActivity.this, null);
