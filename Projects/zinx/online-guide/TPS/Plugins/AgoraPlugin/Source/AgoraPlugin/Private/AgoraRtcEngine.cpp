...
AgoraRtcEngine::AgoraRtcEngine(agora::rtc::IRtcEngine2* rtcEngine)
...
{
	agora::rtc::IRtcEngine2* agoraEngine = static_cast< agora::rtc::IRtcEngine2 * >(::createAgoraRtcEngine());
	if (!agoraEngine)
	{
		return nullptr;
	}
...
int AgoraRtcEngine::adjustUserPlaybackSignalVolume( unsigned int uid, int volume )
{
...
int AgoraRtcEngine::setAudioMixingPitch( int pitch )
{
...
int AgoraRtcEngine::setLocalRenderMode(agora::rtc::RENDER_MODE_TYPE renderMode, agora::rtc::VIDEO_MIRROR_MODE_TYPE mirrorMode)
{
   return m_rtcEngine->setLocalRenderMode(renderMode, mirrorMode);
}
...
int AgoraRtcEngine::setRemoteRenderMode(agora::rtc::uid_t userId, agora::rtc::RENDER_MODE_TYPE renderMode, agora::rtc::VIDEO_MIRROR_MODE_TYPE mirrorMode)
{
   return m_rtcEngine->setRemoteRenderMode(userId, renderMode, mirrorMode);
}
...
int AgoraRtcEngine::enableFaceDetection( bool enable )
{
   return m_rtcEngine->enableFaceDetection( enable );
}
#endif
...
agora::rtc::IChannel* AgoraRtcEngine::createChannel(const char *channelId)
{
   return m_rtcEngine->createChannel(channelId);
}
