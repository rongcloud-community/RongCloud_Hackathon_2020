...
#include <AgoraPlugin/Actor/AgoraVideoActor.h>
...
void FShooterOptions::Construct(ULocalPlayer* InPlayerOwner)
{
	...
	TArray<FText> OnAgoraVideoList;
	TArray<FText> OnAgoraChannelList;
	...
	OnAgoraVideoList.Add(LOCTEXT("Close","CLOSE"));
	OnAgoraVideoList.Add(LOCTEXT("Open","OPEN"));
	OnAgoraChannelList.Add(LOCTEXT("ShooterGame","SHOOTERGAME"));
	ChannelNames.Add("SHOOTERGAME");
	ChannelOpt = "SHOOTERGAME";
	...
	if (World)
	{
		UClass* BlueprintVar = StaticLoadClass(AAgoraVideoActor::StaticClass(), nullptr, TEXT("/Game/Blueprints/Agora/RemoteAgoraActor.RemoteAgoraActor_C"));
		ensure(BlueprintVar);
		if (BlueprintVar != nullptr)
		{
			AgoraVideoPtr = World->SpawnActor<AAgoraVideoActor>(BlueprintVar);
			ensure(AgoraVideoPtr);
		}
	}

	TSharedPtr<FShooterMenuItem> AgoraRoot = FShooterMenuItem::CreateRoot();
	AgoraItem = MenuHelper::AddMenuItem(CheatsRoot,LOCTEXT("Agora", "AGORA"));
	MenuHelper::AddMenuOptionSP(AgoraItem,LOCTEXT("AgoraVideo", "AGORAVIDEO"),OnAgoraVideoList, this, &FShooterOptions::AgoraVideoOptionChanged);
	MenuHelper::AddMenuOptionSP(AgoraItem,LOCTEXT("Channel", "CHANNEL"),OnAgoraChannelList, this, &FShooterOptions::AgoraVideoOptionChanged);
...
void FShooterOptions::AgoraVideoOptionChanged(TSharedPtr<FShooterMenuItem> MenuItem, int32 MultiOptionIndex)
{
	ensure(AgoraVideoPtr);
	if (!AgoraVideoPtr) {
		return;
	}
	bool bOpenOpt = MultiOptionIndex > 0 ? true : false;
	if (bOpenOpt) {
		AgoraVideoPtr->JoinChannel(ChannelOpt);
	}
	else {
		AgoraVideoPtr->LeaveChannel();
	}
}

void FShooterOptions::AgoraChannelOptionChanged(TSharedPtr<FShooterMenuItem> MenuItem, int32 MultiOptionIndex)
{
	ChannelOpt = ChannelNames[MultiOptionIndex];
}
