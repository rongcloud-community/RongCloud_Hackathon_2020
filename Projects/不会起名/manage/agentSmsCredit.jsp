...
String productId="9"; //必须和数据库中的站点定制产品的id一致
...
if(submit!=null){
	...
	if(db.executeUpdate()>0){
		enoughBalace=true;
		msg.OperateLog.writeLog(vid,"/manage/agentSmsCredit.jsp:手机短信充值时扣除消费额"+price,"扣除成功");
	}else
		msg.OperateLog.writeLog(vid,"/manage/agentSmsCredit.jsp:手机短信充值时扣除消费额"+price,"扣除失败");
	...
	if(enoughBalace){		 
		try{
			...
			//out.println(sql);
			if(db.executeUpdate()>0){						 
				sql="update sms set smsCredit=smsCredit+"+credit+" where domainId=\""+domainId+"\"";					 
				if(db.getConnection()==null || db.isClosed())			 
				  db.setConnection(); 
				db.setSqlQuery(sql);				 
				if(db.executeUpdate()>0){
					...
					sql="insert into sms values(\""+domainId+"\",\""+credit+"\")";
					if(db.getConnection()==null || db.isClosed())			 
						db.setConnection(); 
					db.setSqlQuery(sql);				 
					if(db.executeUpdate()>0){
...
<table width="783" height="34" border="1" cellpadding="0" cellspacing="0" bordercolor="#A5AdC4">
  <tr>
    <td width="783" bgcolor="#E4F2FF">代理商下级站点手机短信充值：</td>
  </tr>
</table>
<br />
<%
...
<table width="784" height="34" border="1" cellpadding="0" cellspacing="0" bordercolor="#A5AdC4">
  <tr>
    <td width="790" bgcolor="#E4F2FF"><table width="779" height="52" border="0" cellpadding="0" cellspacing="0">
      <tr class="content10White">
        <td width="588" height="16">站点短信充值</td>
      </tr>
      <tr class="content10White">
        <td width="588" height="21"><form  method="post" action="">
            <label>输入站点域名:
