...
<%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,java.net.*,java.util.*,java.text.SimpleDateFormat" errorPage="" %>
...
String productId="10"; //必须和数据库中的站点定制产品的id一致
...
boolean activated=false; //帐户是否被激活
String cardId=null;  //要充值的用户voip id
submit=request.getParameter("submit");
String submit0=request.getParameter("submit0"); 
if(submit!=null){
	boolean enoughBalace=false;
	//先查看voip是否已经被激活
	sql="select accountId from voip where domainId=\""+domainId+"\"";
	...
	if(activated){			 
		...
		if(db.executeUpdate()>0){
			enoughBalace=true;
			msg.OperateLog.writeLog(vid,"/manage/agentVoipCredit.jsp:voip话费充值时扣除消费额"+price,"扣除成功");
		}else
			msg.OperateLog.writeLog(vid,"/manage/agentVoipCredit.jsp:voip话费充值时扣除消费额"+price,"扣除失败");
	} 
	if(enoughBalace && activated){		 
		try{			 
			String url="http://voip.100im.cn/switch/dc/account_rc_act.asp?cardid="+cardId+"&Balance="+price;			 
			...
			//out.println(hc.getResponseMessage()+" "+url);
			if(re==200){					 
		...
		out.println("<font color='red'>帐户余额不足或帐户VOIP未激活,订单提交失败</font>");
}
%>
<table width="783" height="34" border="1" cellpadding="0" cellspacing="0" bordercolor="#A5AdC4">
  <tr>
    <td width="783" bgcolor="#E4F2FF">代理商下级站点网站短VOIP话费充值：</td>
  </tr>
</table>
<br />
<%
...
<script language="javascript" type="text/javascript">
	function calculateCredte(){
		...
		if(amount!=null){
			if(amount.value<100){
				amount.value=100;
				alert("手机短信充值不能低于100元。");	
			}	
		}
		if(amount!=null && credit!=null)
			credit.value=amount.value * 10;
	}
</script>
<table width="784" height="34" border="1" cellpadding="0" cellspacing="0" bordercolor="#A5AdC4">
  <tr>
    <td width="790" bgcolor="#E4F2FF"><table width="779" height="52" border="0" cellpadding="0" cellspacing="0">
      <tr class="content10White">
        <td width="588" height="16">VOIP话费充值</td>
      </tr>
      <tr class="content10White">
        <td width="588" height="21"><form  method="post" action="">
            <label>输入站点域名:
            ...
            <label>
            <input type="text" name="amount" id="amount" value="100" size="6" onkeyup="calculateCredte();" />            
