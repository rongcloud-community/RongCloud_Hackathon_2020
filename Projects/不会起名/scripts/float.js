...
try{xmlDoc = new ActiveXObject("Microsoft.XMLDOM");}catch(e){xmlDoc = new XMLHttpRequest();  }
 ...
 if(guokeType != '1')
	guoke();
			...
			for( var ni=0;ni<nodelist.length;ni++)
			{
				...
				if(node.nodeName=="YdGroup")
				{
					...
					for( var nj=0;nj<node.childNodes.length;nj++)
					{
						...
						divid=tk_node2.getAttribute("divid");
						mbenabled="false";voipenabled="false";
						if(tk_node2.getAttribute("mbenabled")!=null)
							mbenabled=tk_node2.getAttribute("mbenabled");
						if(tk_node2.getAttribute("voipenabled")!=null)
							voipenabled=tk_node2.getAttribute("voipenabled");					
		...
		{
			loadydcss(frmIP);		
			var node=hostids.XMLDocument.selectSingleNode("//Hosts"),tk_node2;			
			if(node==null)
			{
				initTransfer();
		...
		function HostList(nocreate)
		{
			...
			try
			{
				if(!isall)
				{
					...
					if(floatype=="0")
					{
						if(floatImgType.indexOf("Right")>0){
							xpos=document.body.clientWidth-120;
							window.attachEvent("onresize",resizeWin1);
						}
...
try{xmldoc = new ActiveXObject("Microsoft.XMLDOM"); }catch(e){xmldoc = new XMLHttpRequest(); } 
	...
	function invite(xmlfile,id,content,ydimg)
	{
		...
		var xmlobj		= null;
		inviteId=id;
try{xmlobj = new ActiveXObject("Microsoft.XMLDOM"); }catch(e){ xmlobj = new XMLHttpRequest();} 
		xmlobj.async	= "false";
...
xmldata = getXmlObj();
		...
		{		
			...
			if (this.headAttribute!=null)
			{
				...
				if(floatLogo!=null)
					str = str + "<div style='width:10px;padding:"+(this.headAttribute.selectSingleNode("..//padding").text)+"px;' title=''><table cellpadding='0' cellspacing='0'><tr><td style='cursor:move;' onclick=window.open('http://"+floatLink+"')>"+yd_getImage("http://"+floatLogo)+"</td></tr></table></nobr></div>";
				else
					str = str + "<div style='width:10px;padding:"+(this.headAttribute.selectSingleNode("..//padding").text)+"px;' title=''><table cellpadding='0' cellspacing='0'><tr><td style='cursor:move;' onclick=window.open('http://"+floatLink+"')>"+yd_getImage(this.yd_frm+(this.headAttribute.selectSingleNode("..//dragimage").text))+"</td></tr></table></nobr></div>";
				str = str + "</td>";
				str = str + "</tr>";
				this.strhead	= str;
				str="";
			...
			else if (this.footAttribute!=null)
			{
				...
				if(floatFoot!=null)
					str = str + "<span onclick='"+(this.footAttribute.selectSingleNode("..//link").text)+"' title='"+(this.footAttribute.selectSingleNode("..//text").text)+"' style='cursor:hand;text-align:center;padding:"+(this.footAttribute.selectSingleNode("..//foot/padding").text)+"px;width:100%;height:100%;border:0px solid;'>"+yd_getImage("http://"+floatFoot)+"</span>";
				else
...
var ydlastScrollY=0;
	...
	{
		host=hlist.Host(toid);
		if(host!=null && host.state=="OFFLINE")
		{
...
function resizeWin1()
	{
  	  	document.getElementById("ydbase").style.pixelLeft =document.body.clientWidth - 120; 		 		
	}
//////////////////////////////////
