...
int UIAgora::AdjustUserPlaybackSignalVolume( int64 uid, int volume )
{
   if( !rtcEngine )
      return -1;
...
int UIAgora::SetLocalRenderModeEx( ERENDER_MODE_TYPE renderMode )
{
   if( !rtcEngine )
      return -1;
...
int UIAgora::SetLocalRenderMode( ERENDER_MODE_TYPE renderMode, EVIDEO_MIRROR_MODE_TYPE mirrorMode )
{
   if( !rtcEngine )
      return -1;
   return rtcEngine->setLocalRenderMode( convert( renderMode ), convert( mirrorMode ) );
}

int UIAgora::SetRemoteRenderModeEx( int64 userId, ERENDER_MODE_TYPE renderMode )
{
   if( !rtcEngine )
      return -1;
...
int UIAgora::SetRemoteRenderMode( int64 userId, ERENDER_MODE_TYPE renderMode, EVIDEO_MIRROR_MODE_TYPE mirrorMode )
{
   if( !rtcEngine )
      return -1;
   return rtcEngine->setRemoteRenderMode( userId, convert( renderMode ), convert( mirrorMode ) );
}
...
int UIAgora::SetAudioMixingPitch( int pitch )
{
    #if (defined(__APPLE__) && TARGET_OS_IOS)
      UE_LOG( AgoraEngineLog, Error, TEXT( "UIAgora::SetAudioMixingPitch - This function is not available on iOS device" ) );
      return -1;
    #else
       if( !rtcEngine )
           return -1;
...
UAgoraChannel* UIAgora::CreateChannel( EResult& result, const FString& ChannelId)
{
   UAgoraChannel* channel = NewObject<UAgoraChannel>( this );
   if( !channel )
   {
      result = EResult::Error;
      return nullptr;
   ...
   int rez = channel->Initialize( agora::rtc::ue4::AgoraRtcChannel::createAgoraRtcChannel( rtcEngine->createChannel( TCHAR_TO_ANSI( *ChannelId ) ) ) );
   if( rez < 0 )
   {
      result = EResult::Error;
      return nullptr;
...
int UIAgora::EnableFaceDetection( bool enable )
{
#if defined(__ANDROID__) || (defined(__APPLE__) && TARGET_OS_IOS)
   if( !rtcEngine )
      return false;
    //todo: modify after upgrading library to 3.0.1 and higher
    UE_LOG( AgoraEngineLog, Error, TEXT( "UIAgora::EnableFaceDetection - Not supported in current blugin version" ) );
   //return rtcEngine->enableFaceDetection( enable );
    return false;
#else
   UE_LOG( AgoraEngineLog, Error, TEXT( "UIAgora::EnableFaceDetection - This function is available only on android or iOS device" ) );
   return false;
#endif
}
...
int UIAgora::EnableLoopbackRecording( bool Enabled, const FString& DeviceName )
{
#if (defined(__APPLE__) && TARGET_OS_MAC && !TARGET_OS_IPHONE) || defined(_WIN32)
   if( !rtcEngine )
      return -1;
   return rtcEngine->enableLoopbackRecording( Enabled, TCHAR_TO_ANSI( *DeviceName ) );
#else
   UE_LOG( AgoraEngineLog, Error, TEXT( "UIAgora::EnableLoopbackRecording - This function is available only on Mac and Windows devices" ) );
   return -1;
#endif
}
