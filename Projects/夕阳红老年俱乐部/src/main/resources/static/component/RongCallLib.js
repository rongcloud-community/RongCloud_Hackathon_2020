...
(function (global, factory) {
    ...
    var Reason = {
        CANCEL1: {
            code: 1,
            info: '宸辨柟鍙栨秷宸插彂鍑虹殑閫氳瘽璇锋眰'
        },
        REJECT2: {
            code: 2,
            info: '宸辨柟鎷掔粷鏀跺埌鐨勯€氳瘽璇锋眰'
        },
        HANGUP3: {
            code: 3,
            info: '宸辨柟鎸傛柇'
        ...
        REMOTE_CANCEL11: {
            code: 11,
            info: '瀵规柟鍙栨秷宸插彂鍑虹殑閫氳瘽璇锋眰'
        },
        REMOTE_REJECT12: {
            code: 12,
            info: '瀵规柟鎷掔粷鏀跺埌鐨勯€氳瘽璇锋眰'
        },
        REMOTE_HANGUP13: {
            code: 13,
            info: '閫氳瘽杩囩▼瀵规柟鎸傛柇'
    ...
    /**
     * @param  {object} data
     * @param  {object} data.id 鐢ㄦ埛 id
     * @param  {object} data.stream
     * @param  {object} data.stream.type 濯掍綋娴佺姸鎬�
     * @param  {object} data.stream.mediaStream 濯掍綋娴�
     */
    ...
    var observeRoom = function observeRoom(roomId) {
        rongRTCRoom = new rongRTC.Room({
            id: roomId,
            // joined: , // 鍏朵粬浜哄姞鍏ヤ笉澶勭悊, 宸查€氳繃娑堟伅澶勭悊
            left: removeUser
        });
    };
    ...
    var observeStream = function observeStream() {
        rongRTCStream = new rongRTC.Stream({
            ...
            // unpublished: '', 瀵规柟鍙栨秷鎺ㄦ祦, 涓嶅鐞�, calllib 鍙湁閫€鍑�, 娌℃湁鍙栨秷
    ...
    var getMediaID = function getMediaID(params) {
        ...
        return String(sentTime & 0x7fffffff); //ios o鍙敮鎸乻tring 绫诲瀷
    };
    ...
    // patch c++ SDK 澶氱鏃跺彂閫佷竴鏉℃秷鎭紝浼氬啀鏀跺埌涓€鏉″悓鏍� messageUId 鐨勬秷鎭繖閲岃褰曚竴涓嬪仛鎺掗櫎
    ...
    /*
      鏍规嵁 MessageType 杩斿洖 message 瀵硅薄
      let params = {
          messageType:'TextMessage',
          content: { content: 'hello'}    // 娑堟伅浣�
      };
    ...
    */
    ...
    var Reason$2 = function () {
        // key 锛氱敤鎻忚堪鍜岄敊璇爜缁勬垚锛屾柟渚块€氳繃閿欓敊璇爜鎴栬€呮弿杩拌幏鍙�
        var result = {
            CANCEL1: {
                code: 1,
                info: '宸辨柟鍙栨秷宸插彂鍑虹殑閫氳瘽璇锋眰'
    ...
    var sendHungup = function sendHungup(params, callback) {
        ...
        // 鐐瑰嚮鎸傛柇鎸夐挳瑙﹀彂鏃跺彂閫佹秷鎭紝鎺ュ彈鍒� HungupMessage 鏃朵笉鍙戦€佹秷鎭�
    ...
    var calcTimeout = function calcTimeout(params) {
        ...
        util.forEach(userIds, function (userId) {
            ...
            timer.start(function () {
                // 鎺ユ敹鑰呬负鑷繁鏃跺彂閫� HungupMessage, 鍏朵粬浜哄垯鏈湴鍒涘缓 HungupMessage锛岃涓烘浜哄凡蹇界暐銆佹垨鑰呬笉鍦ㄧ嚎銆�
    ...
    var messageHandler = {
        ...
        HungupMessage: function HungupMessage(message) {
            ...
            delete inviteUsers[senderUserId];
            delete callTimer[senderUserId]; // 鎸傛柇鍦ㄩ個璇�
    ...
    var sendCall = function sendCall(data, callback) {
        ...
        sendCommand$1(params, function (error, result) {
            ...
            //涓诲彨鏂� userId 涓� inviterMessage.sentTime
    ...
    var sendInvite = function sendInvite(data, callback) {
        ...
        sendCommand$1(params, function (error, result) {
            ...
            /*  addUserRelation({  //  缇よ亰 A 宸茬粡鍦ㄦ埧闂达紝 A 閭€璇� B 锛孉鐨刴ediaId鍦ㄧ涓€娆ccept鐨勬椂鍊欏凡缁忕‘瀹氾紒姝� sentTime 鏄疉 閭€璇� B鐨勬椂闂存埑 涓嶅簲璇ユ槧灏勪负 A 鐨刴ediaID銆�
                  sentTime: sentTime,
                  senderUserId: senderUserId
                });*/
    ...
    var CallLib = function CallLib(cfg) {
        ...
        if (!cfg.RongIMLib) {
            throw new Error('璇峰紩鍏ヨ寮曞叆 Web SDK : http://www.rongcloud.cn/docs/web.html#sdk');
        }
